...
qboolean CSQC_AddRenderEdict(prvm_edict_t *ed, int edictnum)
{
	...
	// model light
	if (renderflags & RF_MODELLIGHT && PRVM_clientedictvector(ed, modellight_dir))
	{
		...
		Matrix4x4_Transform3x3(&entrender->inversematrix, PRVM_clientedictvector(ed, modellight_dir), entrender->modellight_lightdir);
